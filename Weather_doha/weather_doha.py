...
def initialize_database():
            conn = sqlite3.connect('Weather_doha/weather_doha.db')
            cursor = conn.cursor()
            cursor.execute('''
            CREATE TABLE IF NOT EXISTS weather_doha_data (
                timestamp DATETIME,
                temperature FLOAT,
                humidity INTEGER,
                weather VARCHAR(40),
...
def fetch_weather_data():
    ...
    if response.status_code == 200:
        ...
        print(weather_doha_data)

        conn = sqlite3.connect('Weather_doha/weather_doha.db')  
        cursor = conn.cursor()
        cursor.execute('''
        INSERT INTO weather_doha_data (
            timestamp,
            temperature,
            humidity,
            weather,
        ...
        ''', (weather_doha_data['timestamp'], weather_doha_data['temperature'], weather_doha_data['humidity'], weather_doha_data['weather'], weather_doha_data['feels_temp']))
